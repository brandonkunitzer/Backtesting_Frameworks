...
class VectorizedBacktester():
    ...
    open so no need to shift position when defining a strategy. For examples of how to define a strategy, check the three examples in the class. This class 
    additionally contains the ability to graph prices and technical indicators. Graphing the indicators is NOT required to test a strategy and can be 
    disregarded if the users only purpose is to test strategies (the adding indicators functions can also be disregarded as the user can add indicators in their
    own function to test a strategy). Since trading costs vary per broker, they are not accounted for in this program, but should be accounted
    ...
    def reset_data(self):
        """
        This function removes all indicators from self.data
        """
        self.data = self.get_data()

    #______________________Add more indicators to graph here ____________________
    ...
    def add_sma(self, window):
        ...
        """
        data = self.data.copy()
        data['sma{}'.format(window)] = data.price.rolling(window).mean()
        self.data = data

    def add_bollinger_bands(self, window, deviations):
        """
        ...
        data = self.data.copy()
        data['upper_sma{}_std{}'.format(window, deviations)] = data['sma{}'.format(window)] + data.price.rolling(window).std() * deviations
        data['lower_sma{}_std{}'.format(window,deviations)] = data['sma{}'.format(window)] - data.price.rolling(window).std() * deviations
        self.data = data
    ...
    def add_rsi(self, window = 14):
        ...
        avg_gain = data['gain'].rolling(window, min_periods=1).mean()
        avg_loss = data['loss'].rolling(window, min_periods=1).mean()
        
        rs = avg_gain / avg_loss
        
        data['rsi'] = 100 - (100 / (1 + rs))
        
        self.data['rsi'] = data.rsi
    ...
    def test_bollinger_bands(self, sma_window, deviations, rsi = False, short = True):
        """
        Test a bollinger bands crossover strategy where you go short when price is above the upper band and you wait until price crosses the sma 
        to go neutral. You go long when price is below the lower band and you wait until the price crosses the sma to go neutral.
        ...
        self.results = results
        if rsi:
            self.recent_strategy = 'Bollinger_bands - RSI | Short SMA Length: {} | Long SMA Length: {}'.format(sma_window, deviations)
        else:
            self.recent_strategy = 'Bollinger_bands | Short SMA Length: {} | Long SMA Length: {}'.format(sma_window, deviations)
        self.plot_results()
    ...
    def plot_indicators(self):
        """
        Plot the indicators that were added to the general chart using the functions designed to add indicators
        """
        data = self.data.copy()
        data = data.loc[self.start : self.end]
        data = data.drop(columns=['Volume', 'returns'])
        
        if 'rsi' in data.columns:
            fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8), sharex=True)
            
            upper = data.drop(columns = ['rsi', 'price', 'Open'])
            # ax1.plot(data.drop(columns = ['rsi']))
            ax1.set_title('Indicators | ticker: {} | start: {} | end: {}'.format(self.symbol, self.start, self.end))
            ax1.set_ylabel('Price')
            ax1.plot(data['price'], label='Price')
            for column in upper.columns:
                line, = ax1.plot(upper[column], label=column, linestyle='--')
            ...
            data['price'].plot(label = 'Price', figsize=(12,8))
            data = data.drop(columns = ['price', 'Open'])
            for column in data.columns:
                line, = plt.plot(data[column], label=column, linestyle='--')
                line.set_label(column)
            plt.legend(loc='upper left')
            plt.title('Indicators | ticker: {} | start: {} | end: {}'.format(self.symbol, self.start, self.end))
            plt.ylabel('Price')
